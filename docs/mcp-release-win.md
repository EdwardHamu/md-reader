...
## 草稿 Release 404 修复（2026-09-19）

现象：NSIS 构建完成后，`publish-win.ps1` 连续 5 次收到 HTTP 404，发布失败。

只读查询确认：`v0.3.10-win.1` 已有草稿（Release ID `391988579`），目标提交 `ba6454331d7eec870e4bb6a8447372a64f289124`，资产为空。未修改或删除该线上草稿。

原因：旧脚本创建 draft 后，通过 `GET /repos/{repo}/releases/tags/{tag}` 查询它；这个接口按标签查询已发布 Release，不适用于这里的草稿查找。重试不能修复错误的查询方式。

修复：

- 使用经过认证的 Release 列表，分页后按完整 `tag_name` 查找，兼容草稿和已发布版本。
- 取得 Release ID 后，通过 `/releases/{id}` 校验上传结果和最终发布状态。
- 保留源码 SHA 校验、已有标签保护、上传重试和摘要校验；没有放宽为忽略所有 404。
- 重试日志现在包含操作/端点，以便区分标签不存在、Release 查询及其他 API 错误。
- 新增 `scripts/test-publish-win.ps1`，在真实发布脚本 AST 中仅替换 gh 传输层，使用模拟 EXE 和临时目录验证实际发布控制流；workflow 在构建前运行这些离线测试。

验证：Windows 本机 8/8 发布控制流测试通过，覆盖新建草稿、已有草稿、已有公开版本、创建响应丢失、标签源码冲突、草稿源码冲突、摘要不匹配及创建后不可见。原 HEAD 的旧发布脚本在同一测试中重现草稿 HTTP 404，修复版通过。`git diff --check` 通过。没有在本机实际运行 CI 的 PowerShell 7 进程传输层，也未重新执行线上构建/上传。

重试方法：先手动提交修复（包括新增测试和 workflow），再运行自动推送/监控脚本。直接点击旧运行的 Re-run jobs 会仍使用旧的固定源码提交，不会使用本地修复。

```bash
bash scripts/release-win.sh --ref main --tag v0.3.10-win.2
```
